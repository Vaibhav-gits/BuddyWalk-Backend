...
exports.createGroup = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        console.error("createGroup insert error:", err);
      ...
      db.query(
        ...
        (err2) => {
          if (err2) {
            console.error("createGroup member error:", err2);
        ...
        (err2, existing) => {
          ...
          if (existing.length > 0) {
            db.query(
              ...
              [invitationId],
              () => res.json({ message: "Already a member of this group." }),
            );
            return;
          }
...
exports.leaveGroup = (req, res) => {
  ...
  db.query(
    `SELECT u.name AS leaver_name, g.name AS group_name
...
exports.getGroupLeaderboard = (req, res) => {
              ...
              db.query(
                ...
                (e3) => {
                  if (e3)
                    return res
                      .status(500)
                      .json({ message: "DB error", error: e3 });

                  db.query("DELETE FROM grp WHERE id = ?", [groupId], (e4) => {
                    if (e4)
                      return res
                        .status(500)
                        .json({ message: "DB error", error: e4 });
